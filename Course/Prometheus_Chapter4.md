...
پرومتئوس برای انتخاب آخرین نمونه از سری‌های زمانی، به‌طور پیش‌فرض تا 5 دقیقه به عقب نگاه می‌کند (دلتای نگاه به عقب). این تنظیم از نیاز به به‌روزرسانی‌های مداوم (مثلاً هر چند ثانیه) جلوگیری می‌کند و در عین حال سری‌های زمانی منسوخ (مانند سری‌هایی که یک روز پیش متوقف شده‌اند) را حذف می‌کند.
...
پرومتئوس می‌تواند تشخیص دهد که یک سری زمانی در زمان خاصی متوقف شده است (مثلاً وقتی یک هدف دیگر در اسکریپ بعدی گزارش نشود یا هدف کاملاً غیبش بزند). در این موارد، یک **علامت منسوخ بودن (staleness marker)** در پایگاه داده زمانی (TSDB) ثبت می‌شود.

- انتخابگر بردار لحظه‌ای سری‌هایی را که آخرین نمونه آنها علامت منسوخ بودن است، فوراً از خروجی حذف می‌کند، حتی اگر نمونه‌های دیگری در بازه 5 دقیقه‌ای وجود داشته باشند.
...
demo_memory_usage_bytes
```
اگر یکی از سری‌ها علامت منسوخ بودن داشته باشد، در خروجی ظاهر نمی‌شود، حتی اگر نمونه‌های قدیمی‌تری در بازه 5 دقیقه‌ای وجود داشته باشند.
