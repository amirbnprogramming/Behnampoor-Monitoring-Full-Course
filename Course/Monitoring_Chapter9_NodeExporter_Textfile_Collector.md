...
### مشکل
Node Exporter به‌صورت پیش‌فرض متریک‌های استاندارد سیستم (مانند CPU، حافظه، دیسک، و شبکه) را جمع‌آوری می‌کند، اما بسیاری از سناریوها نیاز به متریک‌های خاص دارند که در کلکتورهای پیش‌فرض وجود ندارند. مثلاً:
...
- Node Exporter این فایل‌ها را بخواند و متریک‌ها را به خروجی خود اضافه کند.
- پرومتئوس بتواند این متریک‌ها را مانند سایر متریک‌های Node Exporter اسکرپ کند.
...
## پیش‌نیازها
- Node Exporter روی سیستم نصب شده باشد (مراجعه به قسمت هشتم آموزش).
- پرومتئوس برای اسکرپ متریک‌ها تنظیم شده باشد.
- گرافانا (اختیاری) برای بصری‌سازی متریک‌ها.
- دسترسی به خط فرمان لینوکس و دانش اولیه درباره نوشتن اسکریپت.

## گام ۱: فعال‌سازی Textfile Collector

Textfile Collector به‌صورت پیش‌فرض در Node Exporter فعال است، اما باید یک دایرکتوری برای ذخیره فایل‌های `.prom` مشخص کنید.
...
2. **پیکربندی Node Exporter:**
   ...
   ```ini
   ExecStart=/usr/local/bin/node_exporter --collector.textfile.directory=/var/lib/node_exporter/textfile_metrics
   ```
...
```
- **HELP:** توضیحی برای متریک.
- **TYPE:** نوع متریک (مثل `gauge`، `counter`، یا `histogram`).
- **value:** مقدار عددی متریک.
...
3. **هشدار در پرومتئوس:**
   ```promql
   ALERT RaidNotOptimal
     IF node_raid_status < 1
     FOR 5m
     LABELS { severity = "critical" }
     ANNOTATIONS {
       summary = "مشکل در وضعیت RAID",
       description = "حداقل یک آرایه RAID در وضعیت غیر بهینه است."
     }
   ```
