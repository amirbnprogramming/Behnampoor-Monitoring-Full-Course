# آموزش مانیتورینگ : قسمت دهم نمایش هیستوگرام‌های پرومتئوس به‌صورت هیت‌مپ در گرافانا
...
برای نمایش هیستوگرام، باید یک کوئری PromQL بنویسیم که داده‌های هیستوگرام را از پرومتئوس استخراج کند.
به‌عنوان مثال، فرض کنید متریکی به نام `http_request_duration_seconds` داریم که تأخیر درخواست‌های HTTP را در قالب هیستوگرام ذخیره می‌کند.
...
**توضیح کوئری**:
- مترک `demo_api_request_duration_seconds_bucket`: این متریک، تعداد درخواست‌ها را در بازه‌های مختلف تأخیر (buckets) ذخیره می‌کند.
- تابع `rate(...[5m])`: نرخ درخواست‌ها در هر ثانیه را در بازه زمانی ۵ دقیقه محاسبه می‌کند.
...
پس از اجرای کوئری، ممکن است هیت‌مپ به‌درستی نمایش داده نشود.
دلیل آن، تنظیمات پیش‌فرض گرافانا است که فرض می‌کند داده‌های هیستوگرام غیرتجمعی هستند. برای رفع این مشکل:
...
> **چرا این مهم است؟**  
> هیستوگرام‌های پرومتئوس به‌صورت پیش‌فرض داده‌های تجمعی را ذخیره می‌کنند (یعنی هر بازه شامل تمام درخواست‌های تا آن حد است).
>  گرافانا باید این رفتار را به‌درستی تفسیر کند تا هیت‌مپ دقیق باشد.
...
<img width="1271" height="1123" alt="image" src="https://github.com/user-attachments/assets/267a3773-dfbf-421c-921e-0a7dc9dc43b9" />
