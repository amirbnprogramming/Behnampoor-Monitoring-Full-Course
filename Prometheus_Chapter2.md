...
**تحلیل عمیق:**
- **scrape_interval**: این پارامتر مشخص می‌کند که پرومتئوس با چه فواصلی از اهداف خود متریک‌ها را جمع‌آوری کند. در محیط‌های تولیدی، معمولاً از فواصل ۱۵ تا ۶۰ ثانیه استفاده می‌شود، زیرا فواصل کوتاه‌تر (مانند ۵ ثانیه) بار پردازشی زیادی ایجاد می‌کنند.
...
**تحلیل عمیق:**
- **job_name**: این برچسب (label) به متریک‌های جمع‌آوری‌شده اضافه می‌شود و برای گروه‌بندی اهداف مشابه (مانند نمونه‌های مختلف یک سرویس) استفاده می‌شود. انتخاب نام‌های معنادار برای `job_name` در محیط‌های بزرگ مهم است.
- **static_configs**: در اینجا، ما آدرس‌های اهداف را به‌صورت دستی (hard-coded) مشخص می‌کنیم. در سناریوهای پیشرفته‌تر، از **service discovery** (مانند DNS یا Consul) برای کشف خودکار اهداف استفاده می‌شود.
