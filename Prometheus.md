...
## نیاز به مانیتورینگ
- **تشخیص مشکلات قبل از وقوع فاجعه**: مانیتورینگ به تیم‌ها اجازه می‌دهد تا مشکلات احتمالی مانند قطعی سرویس، مصرف بیش از حد منابع یا خطاهای نرم‌افزاری را قبل از تأثیرگذاری بر کاربران شناسایی کنند.
- **بهینه‌سازی عملکرد**: با جمع‌آوری داده‌های عملکرد، تیم‌ها می‌توانند نقاط ضعف سیستم را شناسایی کرده و برای بهبود آن اقدام کنند.
- **تضمین تجربه کاربری**: در دنیای رقابتی امروز، ارائه خدمات پایدار و باکیفیت به کاربران از اهمیت بالایی برخوردار است. مانیتورینگ کمک می‌کند تا تجربه کاربری بهینه حفظ شود.
- **پشتیبانی از تصمیم‌گیری داده‌محور**: داده‌های مانیتورینگ به مدیران و مهندسان کمک می‌کند تا تصمیمات مبتنی بر داده‌های واقعی بگیرند.
- **مدیریت پیچیدگی‌های معماری‌های مدرن**: با ظهور معماری‌هایی مانند میکروسرویس‌ها، نیاز به نظارت بر اجزای متعدد و توزیع‌شده سیستم‌ها افزایش یافته است.
...
بیایم هر کدوم رو با جزئیات بررسی کنیم و وظایفشون رو توضیح بدیم.

---

### ۱. سرور مانیتورینگ (Monitoring Server)
**وظیفه**: قلب سیستم Pull-based هست و مسئول مدیریت کل فرآیند مانیتورینگ، از جمع‌آوری متریک‌ها تا ذخیره و پردازششونه.
...
- می‌تونه چندین نمونه (instance Chaudharyاز خودش رو برای مقیاس‌پذیری اجرا کنه (مثلاً تو سازمان‌های بزرگ).
- معمولاً یه رابط وب ساده برای مشاهده متریک‌ها و کوئری کردن داده‌ها داره.

**مثال**: تو پرومتئوس، سرور پرومتئوس این نقش رو بازی می‌کنه و از طریق فایل پیکربندی (prometheus.yml) هدف‌ها و تنظیمات scrape رو مدیریت می‌کنه.
...
**وظیفه**: ذخیره متریک‌های جمع‌آوری‌شده به‌صورت سری‌های زمانی برای استفاده‌های بعدی مثل کوئری، تحلیل و هشداردهی.

**جزئیات**:
- TSDB داده‌ها رو به‌صورت سری‌های زمانی ذخیره می‌کنه. هر سری زمانی یه شناسه یکتا داره (ترکیب نام متریک و لیبل‌ها) و شامل یه لیست از نمونه‌ها (تایم‌استمپ + مقدار) هست.
...
- TSDB معمولاً روی دیسک محلی سرور مانیتورینگ ذخیره می‌شه، ولی می‌تونه به سیستم‌های ذخیره‌سازی خارجی هم متصل بشه.
...
**مثال**: تو پرومتئوس، TSDB داده‌ها رو به‌صورت سری‌های زمانی مثل `http_requests_total{status_code="200",instance="web1.example.com:8080"}` ذخیره می‌کنه، و هر نمونه یه تایم‌استمپ و مقدار داره.
...
- هشدارها معمولاً از طریق یه کامپوننت جداگونه (مثل Alertmanager تو پرومتئوس) مدیریت، جمع‌بندی و به مقصدهایی مثل ایمیل، Slack یا PagerDuty ارسال می‌شن.
...
**مثال**: تو پرومتئوس، Alertmanager هشدارها رو بر اساس قوانین PromQL مدیریت می‌کنه. مثلاً اگه `rate(http_requests_total{status_code="500"}[5m]) > 0.1` باشه، یه هشدار برای خطاهای 500 می‌فرسته.
...
فرض کنید یک معلم داریم و یک دانش آموز در حال دادن امتحان ... در معماری پول بیس معلم هر چند لحظه یکبار می آید داده‌هایی رو از وضعیت دانش آموز جمع میکند ( چجوری نشسته – تقلب میکنه یا نه – تموم کرده یا نه – نیاز به کمک داره یا ...) با فرض اینکه خود دانش آموز یا برگه امتحانیش این داده‌ها رو به صورتی که برای معلم خوانا هست داره تولید میکنه برای اینکه تصویر کامل بشه، بیایم روال کار رو قدم‌به‌قدم مرور کنیم:

1. **کشف هدف‌ها**: سرور مانیتورینگ از طریق فایل پیکربندی یا کشف سرویس، لیست هدف‌ها رو پیدا می‌کنه.
2. **جمع‌آوری متریک‌ها**: سرور به‌صورت دوره‌ای به انتهای HTTP هدف‌ها درخواست می‌فرسته و متریک‌ها رو scrape می‌کنه. اگه هدف به‌صورت بومی متریک تولید نکنه، اکسپورتر این کار رو انجام می‌ده.
3. **اضافه کردن لیبل‌ها**: سرور لیبل‌های هدف (مثل instance و job) رو به متریک‌ها اضافه می‌کنه خود اکسپورتر در هدف یا وابسته به هدف لیبل‌هایی رو که می‌خواهد بر اساس آنها تحلیل یا دسته‌بندی صورت بگیره برای سری زمانی‌ای که قرار است تولید شود می‌سازد و سرور هم دو لیبل خاص و ویژه مذکور رو اضافه می‌کنه.
...
## مدل داده پرومتئوس
پرومتئوس از یک مدل داده چندبعدی استفاده می‌کند که شامل:

- **نام معیار (Metric Name)**: نشان‌دهنده چیزی که اندازه‌گیری می‌شود (مانند تعداد درخواست‌های HTTP).
- **برچسب‌ها (Labels)**: جفت‌های کلید-مقدار که معیارها را به‌صورت دقیق‌تر تفکیک می‌کنند (مانند `method="GET"` یا `status_code="200"`).
- **نمونه‌ها (Samples)**: شامل یک مقدار عددی (64-bit float) و یک برچسب زمانی (Timestamp) برای هر معیار.

---

## زبان پرس‌وجو (PromQL)
PromQL زبان پرس‌وجوی پرومتئوس است که امکان تحلیل پیچیده داده‌های زمانی را فراهم می‌کند. این زبان برای محاسبات مبتنی بر سری‌های زمانی طراحی شده و قابلیت‌هایی مانند:
...
## پایگاه داده زمانی (TSDB)
پایگاه داده زمانی پرومتئوس (TSDB) برای ذخیره سری‌های زمانی بهینه‌سازی شده است. این پایگاه داده:
- داده‌ها را به‌صورت محلی ذخیره می‌کند و برای نگه‌داری داده‌ها زمان انقضا هم در نظر می‌تواند بگیرد.
- از فشرده‌سازی کارآمد برای کاهش فضای ذخیره‌سازی استفاده می‌کند.
- امکان ارسال داده‌ها به سیستم‌های ذخیره‌سازی راه دور را نیز دارد.
...
## مثال عملی با پرومتئوس
برای اینکه همه‌چیز ملموس‌تر بشه، بیایم یه سناریو با پرومتئوس تصور کنیم:

**سناریو**: یه سرور وب (هدف) با آدرس `web1.example.com:8080` داری که متریک `http_requests_total{status_code="200"}` رو تو `/metrics` ارائه می‌ده.

- **سرور پرومتئوس**: تو فایل `prometheus.yml` این هدف رو تعریف کردی و هر 15 ثانیه scrape می‌کنه.
- **اکسپورتر (در صورت نیاز)**: اگه سرور وب متریک تولید نکنه، یه اکسپورتر کنارش اجرا می‌کنی.
- **TSDB**: متریک‌ها به‌صورت سری زمانی مثل `http_requests_total{status_code="200",instance="web1.example.com:8080",job="web_server"}` ذخیره می‌شن.
- **کشف سرویس**: اگه تو Kubernetes باشی، پرومتئوس از API سرور Kubernetes هدف‌ها رو پیدا می‌کنه.
- **هشداردهی**: یه قانون هشدار تعریف کردی که اگه `rate(http_requests_total{status_code="500"}[5m]) > 0.1` باشه، هشدار به Slack بفرسته.
- **تجسم**: داده‌ها رو تو Grafana به‌صورت نمودار نمایش می‌دی.
